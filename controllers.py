...
def register_tutor():
    if request.method == "POST":
        ...
        user=User.query.filter_by(registration_number=reg_number, role="tutor").first()
        if user:
        ...
        new_tutor=Tutor(registration_number=reg_number+"_tutor", subject=subject, cgpa=cgpa, description=description, grade_history=file_path)
        db.session.add(new_tutor)
        db.session.commit()

        password_hash=generate_password_hash(password)
        new_user=User(registration_number=reg_number+"_tutor", name=name, email=email, password_hash=password_hash, role="tutor")
...
def register_student():
    if request.method == "POST":
        ...
        user=User.query.filter_by(registration_number=reg_number, role="student").first()
        if user:
            flash("User already exists")
            return redirect(url_for('register_student'))
        ...
        new_student=Student(registration_number=reg_number, year_of_study=year, id_card_image=file_path)
        db.session.add(new_student)
        db.session.commit()

        password_hash=generate_password_hash(password)
        new_user=User(registration_number=reg_number, name=name, email=email, password_hash=password_hash, role="student")
        db.session.add(new_user)
        db.session.commit()
        flash("Registered successfully")
